# Photography
Real Estate HDR Photography Edit Portfolio

[![License](https://img.shields.io/badge/License-All%20Rights%20Reserved-red.svg)](LICENSE)
[![Usage](https://img.shields.io/badge/Usage-Permission%20Required-orange.svg)](LICENSE)
[![Editing](https://img.shields.io/badge/Modifications-Strictly%20Forbidden-critical.svg)](LICENSE)
[![Commercial](https://img.shields.io/badge/Commercial%20Use-Paid%20License%20Only-red.svg)](LICENSE)

High Dynamic Range (HDR) Real Estate & Architectural Photography by **[Your Full Name]** ([@[YourSocialHandle]](https://yourwebsite.com))

---

## 🖼️ Featured Work Preview

![Outdoor HDR Preview](Real Estate HDR Edit/DSC04101.jpg)
*Sample HDR Bracket Blend — Preview Only (Watermarked)*

---

## 📜 License & Copyright Notice — ALL RIGHTS RESERVED

**© 2026 Anasimos Mesfin Kebede. All Rights Reserved.**

> ⚠️ **NOTICE TO REALTORS, BUILDERS, DESIGNERS & PLATFORMS:**  
> All photographs in this repository are **protected intellectual property**.  
> Downloading, reposting, editing, or commercial usage without a signed agreement is strictly prohibited.

### 🚫 You MAY NOT:
- ❌ **Use these photos for MLS listings, Airbnb, VRBO, or websites** without prior written permission
- ❌ **Pass these photos to third parties** (builders, stagers, architects, other agents)
- ❌ **Re-edit, crop, apply Instagram/VSCO filters**, or alter color grading
- ❌ **Crop out watermarks or strip EXIF copyright metadata**
- ❌ **Feed these images into AI generators** or training datasets
- ❌ **Claim authorship** of the photographs

### ✅ You MAY:
- 👁️ View these sample images for portfolio evaluation
- 📩 **Request a commercial or residential marketing license**

---

## 📩 Licensing Inquiries & Booking

To license existing images or book a real estate photography shoot:

📧 **Email:** `anasimosmesmesfinkebede@gmail.com`  

Please specify:
1. Property address & intended use (MLS, Airbnb, Architect Portfolio, etc.)
2. Duration of licensing needed
3. Desired format (Web-optimized JPEG, Print TIFF, or HDR brackets)

---

📄 **[Read Full Legal Terms → LICENSE](LICENSE)**
